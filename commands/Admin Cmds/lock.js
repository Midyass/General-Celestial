...
module.exports = {
    name: "lock",
    description: "Lock the channel so members can't send messages.",
        ...
        try {
            ...
            if (perms && perms.deny.has(PermissionsBitField.Flags.SendMessages)) {
                return message.reply({
                    embeds: [new EmbedBuilder()
                        .setColor("Yellow")
                        .setAuthor({ name: message.author.username, iconURL: message.author.displayAvatarURL() })
                        .setDescription("<:warning_26a0fe0f:1335402182177984654> This channel is already locked.")
                    ]
                });
            }
            ...
            return message.reply({
                embeds: [new EmbedBuilder()
                    .setColor("Green")
                    .setAuthor({ name: message.author.username, iconURL: message.author.displayAvatarURL() })
                    .setDescription(`<:true:1348715550477652080> Successfully locked ${channel}.`)
                ]
            });
            ...
            return message.reply({
                embeds: [new EmbedBuilder()
                    .setColor("Yellow")
                    .setAuthor({ name: message.author.username, iconURL: message.author.displayAvatarURL() })
                    .setDescription("<:warning_26a0fe0f:1335402182177984654> An error occurred while trying to lock the channel.")
                ]
            });
        }
    }
