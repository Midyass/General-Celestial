        ...
        embeds: [
          new EmbedBuilder()
            ...
            .addFields(
              ...
              { name: "Example:", value: `- &addrole <@${message.author.id}> \`roleId\`\n- &addrole \`${message.author.id}\` \`roleId\``, inline: false }
            )
      ...
      return message.reply({
        embeds: [
          new EmbedBuilder()
            .setColor("Yellwo")
            .setAuthor({ name: message.author.username, iconURL: message.author.displayAvatarURL() })
            .setDescription(`<:warning_26a0fe0f:1335402182177984654> The user <@${target.id}> already has the role <@&${role.id}>.`)
        ]
      });
    }
      ...
      return message.reply({
        embeds: [
          new EmbedBuilder()
            .setColor("Green")
            .setAuthor({ name: message.author.username, iconURL: message.author.displayAvatarURL() })
            .setDescription(`<:yes:1357389908209307821> Successfully added <@&${role.id}> to <@${target.id}>.`)
        ]
      });
    } catch (err) {
      console.error(err);
