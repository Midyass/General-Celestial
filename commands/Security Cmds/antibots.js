...
module.exports = {
  name: "anti-bot",
  aliases: ["antibots", "antibot", "antibots"],
  description: "Enable or disable the Anti-Bot feature.",
  ...
  async execute(message, args, client) {
    ...
    if (!["on", "off", "status"].includes(option)) {
      const embed = new EmbedBuilder()
        ...
        .addFields(
          { name: "__Aliases__ :", value: "- `antibots`, `antibots`", inline: false },
    ...
    const antiBotEnabled = settings.antiBot === 1;

    if (option === "status") {
      const statusEmbed = new EmbedBuilder()
        .setColor(antiBotEnabled ? "#00b7ff" : "#00b7ff")
        .setAuthor({ name: 'Anti Bot : Status', iconURL: message.author.displayAvatarURL({ dynamic: true }) })
        .addFields(
          { name: "__Anti Bot Status__ :", value: antiBotEnabled ? "Enabled <:On:1349698525360947241>" : "Disabled <:Off:1349698291259932735>", inline: false }
        )
        .setThumbnail(message.author.displayAvatarURL({ dynamic: true }));
  ...
  async onMessage(message, client) {
    ...
    const settings = await client.getSettings(message.guild.id);
    if (!settings || settings.antiBot === 0) return;

    const botRegex = /bot/gi;
    if (botRegex.test(message.content)) {
      try {
